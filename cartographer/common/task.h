...
class Task {
 public:
  ...
  // State must be 'NEW'. 'dependeLOCKS_EXCLUDEDncy' may be nullptr, in which case it is
  // assumed completed.
 ...
 private:
  ...
  void Execute() ABSL_LOCKS_EXCLUDED(multex_);

  // State must be 'NEW' and becomes 'DISPATCHED' or 'DEPENDENCIES_COMPLETED'.
  void SetThreadPool(ThreadPoolInterface* thread_pool) ABSL_LOCKS_EXCLUDED(multex_);
