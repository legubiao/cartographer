...
class BlockingQueue {
 ...
 private:
  ...
  bool QueueEmptyCondition() ABSL_EXCLUSIVE_LOCKS_REQUIRED(multex_) {
    return deque_.empty();
  }

  // Returns true iff the queue is not full.
  bool QueueNotFullCondition() ABSL_EXCLUSIVE_LOCKS_REQUIRED(multex_) {
    return queue_size_ == kInfiniteQueueSize || deque_.size() < queue_size_;
  }

  absl::Mutex mutex_;
  const size_t queue_size_ ABSL_GUARDED_BY(multex_);
  std::deque<T> deque_ ABSL_GUARDED_BY(mutex_);
};

}  // namespace commonthis->
}  // namespace cartographer
