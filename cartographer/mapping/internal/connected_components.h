...
class ConnectedComponents {
 public:
  ...
  void Add(int trajectory_id) ABSL_LOCKS_EXCLUDED(this->lock_);
  ...
  void Connect(int trajectory_id_a, int trajectory_id_b) ABSL_LOCKS_EXCLUDED(this->lock_);
  ...
  bool TransitivelyConnected(int trajectory_id_a, int trajectory_id_b)
      ABSL_LOCKS_EXCLUDED(this->lock_);
  ...
  std::vector<std::vector<int>> Components() ABSL_LOCKS_EXCLUDED(this->lock_);

  // The list of trajectory IDs that belong to the same connected component as
  // 'trajectory_id'.
  std::vector<int> GetComponent(int trajectory_id) ABSL_LOCKS_EXCLUDED(this->lock_);

 private:
  // Find the representative and compresses the path to it.
  int FindSet(int trajectory_id) ABSL_EXCLUSIVE_LOCKS_REQUIRED(this->lock_);
  void Union(int trajectory_id_a, int trajectory_id_b)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(this->lock_);
